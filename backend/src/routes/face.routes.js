...
router.post('/verify', authenticate, async (req, res, next) => {
    try {
        ...
        if (Array.isArray(storedFaceData)) {
            // Direct array format
            if (storedFaceData.length === 128 && storedFaceData.every(v => typeof v === 'number')) {
                storedDescriptors = [storedFaceData];
            } else if (storedFaceData.length > 0 && Array.isArray(storedFaceData[0])) {
                // Array of arrays
                storedDescriptors = storedFaceData;
            }
        } else if (storedFaceData && typeof storedFaceData === 'object') {
            // Object format - check different property names
            if (storedFaceData.descriptor && Array.isArray(storedFaceData.descriptor)) {
        ...
        if (storedDescriptors.length === 0) {
            console.error('Invalid stored face data structure:', storedFaceData);
            return res.status(400).json({
                success: false,
        ...
        if (bestDistance === Infinity) {
            return res.status(400).json({
                success: false,
                error: 'Could not calculate face distance. Please re-register your face.'
            });
        }
