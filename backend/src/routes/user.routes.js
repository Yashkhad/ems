...
router.post('/', authenticate, canManageUsers, userValidation.create, async (req, res, next) => {
    try {
        ...
        const {
            ...
            role, department_id, shift_id, manager_id, date_of_joining, joining_date
        } = req.body;
        ...
        if (finalDepartmentId) {
            ...
            if (dept) {
                ...
                let nextNumber = 1;
                if (lastGeneric) {
                    const numPart = lastGeneric.employeeId.replace('EMS', '');
                    nextNumber = parseInt(numPart, 10) + 1;
                }
...
router.put('/:id', authenticate, canManageUsers, userValidation.update, async (req, res, next) => {
    try {
        ...
        // Support is_active boolean (sent from toggle button in frontend)
        ...
        if (updates.date_of_joining !== undefined) updateData.dateOfJoining = new Date(updates.date_of_joining);
        if (updates.joining_date !== undefined) updateData.dateOfJoining = new Date(updates.joining_date);
