  ...
  const handleFaceVerification = async () => {
    ...
    try {
      ...
      if (!Array.isArray(faceDescriptor) || faceDescriptor.length !== 128) {
        ...
        toast.error('Invalid face descriptor detected. Please try again.')
        setIsVerifying(false)
        return
      }
      ...
      if (verifyResponse.data.success && verifyResponse.data.data.verified) {
        ...
        toast.success(`Face verified with ${(faceScore * 100).toFixed(1)}% confidence!`)
        ...
        const distance = verifyResponse.data.data.distance
        console.warn('Face verification failed:', {
          distance: distance,
          threshold: verifyResponse.data.data.threshold,
          score: verifyResponse.data.data.score
        })
        toast.error(`Face verification failed (distance: ${distance}). Please try again with better lighting.`)
      }
    } catch (error) {
      console.error('Verification error:', error)
      setVerificationResult('failed')
      const errorMsg = error.response?.data?.error || error.message || 'Face verification failed'
